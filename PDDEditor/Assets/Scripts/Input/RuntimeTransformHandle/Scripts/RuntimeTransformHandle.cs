...
namespace RuntimeHandle
{
    ...
    public class RuntimeTransformHandle : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (target == null)
                target = this.GetComponent<PDDMovable>();
            ...
            isDraggingHandle.AddListener(OnMove);
            endedDraggingHandle.AddListener(OnMoveEnd);
        }
        ...
        void Update()
        {
            ...
            if (target == null) { for (int i = 0; i < this.transform.childCount; i++) { this.transform.GetChild(i).gameObject.SetActive(false); } }
            else { for (int i = 0; i < this.transform.childCount; i++) { this.transform.GetChild(i).gameObject.SetActive(true); } }
        }
        ...
        static public RuntimeTransformHandle Create(PDDMovable p_target, HandleType p_handleType)
        {
        ...
        public void SetTarget(PDDMovable newTarget)
        {
            if (!newTarget) { target = null; };
            target = newTarget;
            if (newTarget.GetType() == typeof(Node))
            {
                Node node = (Node)newTarget;
                if (!node.Item.PDDBaseSettings.Locked)
                {
                    target = newTarget;
                }
                else
